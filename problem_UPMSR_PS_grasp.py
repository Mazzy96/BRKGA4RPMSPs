...
class UPMSR_PS_grasp_instance(object):
    def __init__(self):
        ...
        # self.initial_setup_times = None  # Initial setup times for each machine
...
class UPMSR_PS_grasp(Problem):
    ...
    def evaluate(self, key, print_sol=False):
        ...
        machine_job_sequence, machine_loads = self.local_search(machine_job_sequence, machine_loads, key)
        ...
        current_value = metric['makespan']

        # Compare the current value with the previous value
        if current_value < self.previous_value:
            self.previous_value = current_value  # Update the previous value
            self.best_main_result = machine_job_sequence  # Save the current metrics as the best metrics
    ...
    def evaluate_final(self, machine_job_sequence):
        # Job, Machine, Worker schedules
        JS = []  # Job start times
        JM = []  # Machine assignments
        ...
        # post_metrics = self.calculate_post_metrics(setup_times, process_times, machine_loads, machine_job_sequence, JM)

        # Compute final value using weights
        current_value = metric['makespan']
        
        main_result = {'best_objective_found':current_value, 'machine_job_sequence': machine_job_sequence, 'machine_loads': machine_loads, 'process_times': process_times,
        ...
        excel_file_path = f"Results\\UPMSR_PS_grasp\\{instance_name}_{self.seed}.xlsx"
        ...
        while len(job_order) > 0:
            ...
            for job_idx, job in enumerate(job_order_np):
                for machine_idx in range(self.n_machines):
                    ...
                    combined_matrix[job_idx, machine_idx] = linear_combination
    ...
    def local_search(self, machine_job_sequence, machine_loads, key):
        ...
        while no_improvement_iterations < 50:
            ...
            for machine_idx in range(self.n_machines):
                for position in range(len(machine_job_sequence[machine_idx]) + 1):
                    ...
                    # print("jobs_on_machine", jobs_on_machine)
