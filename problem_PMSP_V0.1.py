...
def read_instance(inst_path):
    ...
    for line in lines:
        ...
        if current_section == "Instance Info":
            ...
            inst.instance_seed = int(lines[15].split(":")[1].strip())
...
class PMSP(Problem):
    def __init__(self, args, inst):
        ...
        self.inst_seed = int(inst.instance_seed)
    ...
    def evaluate(self, key, print_sol=False):
        ...
        # print('machine_flags:', machine_flags)

        metrics = self.calculate_metrics(JS, start_times, end_times, setup_times_start, setup_times_end)

        # current_value = metrics['total_production_time'] + metrics['total_earliness'] + metrics['total_tardiness'] + 100*total_transitions
    ...
    def evaluate_final(self, key):
        ...
        # current_value = metrics['total_production_time'] + metrics['total_tardiness'] + metrics['total_earliness'] + 100*total_transitions
        ...
        excel_file_path = f"Results\\PMSP\\J{self.n_jobs}_M{self.n_machines}_P{self.n_workers}_W{self.n_weeks}_TW{self.tw_density}_ME{self.m_eligibility}_AP{self.n_unavb_worker}_Seed{self.inst_seed}_Seed{self.seed}.xlsx"
    ...
    def assign_jobs_to_machines(self, job, JS, JM, JW, start_times, end_times, worker_start_times, worker_end_times, setup_times_start, setup_times_end, machine_loads, worker_availability, last_jobs_on_machine, last_worker_on_machine, last_machine_for_worker, machine_flags, transitions_per_week):
        ...
        while remaining_machines:  # Continue as long as there are machines left to assign the job
            ...
            for machine in remaining_machines:
                ...
                # potential_makespan_increase = max(machine_loads[machine_idx], self.ready_times[job]) + self.processing_times[job][machine_idx]  + setup_time # Makespan Minimization
        ...
        # Update main lists only once a valid machine is found or forced assignment is made
            ...
            if transfer:
                transfer_setup = self.initial_setup_times[job][machine_idx]

            if machine_loads[machine_idx] == 0:
                assigned_setup_time = 0

            setup_end = setup_start + assigned_setup_time + transfer_setup
    ...
    def assign_worker_to_machine(self, last_worker_on_machine, worker_availability, machine_makespan, assigned_processing_time, assigned_setup_time):
        ...
        #     avail_time = worker_availability[worker_idx]
        ...
        #     difference [worker_idx] = avail_time
    ...
    def calculate_worker_transitions(self, JS, JM, JW, setup_times_start, end_times):
        ...
        # last_setup_start_time_on_machine = [-1] * self.n_machines  # Initialize last setup start time on each machine
