...
# Replication seeds (like Code A)
seeds = list(range(1, 3))   # 0..205 inclusive

# Parallel workers (use 16 threads)
N_WORKERS = 14

# Functions
def generateJobProcessingTimes(min_processing_time, max_processing_time, mean_value, n_machines, n_jobs, machine_eligibility, rng):
    ...
    job_proc_times = rng.integers(low=min_processing_time, high=max_processing_time, size=(n_machines, n_jobs)) # Discrete Uniform distribution

    job_initial_setup_times = generateInitialSetupTimes(n_machines, n_jobs, mean_value)

    machine_eligibilities = generateMachineEligibilities(n_machines, n_jobs, machine_eligibility, rng)
...
def generateMachineEligibilities(n_machines, n_jobs, eligibility_constraint, rng):
    ...
    for job in jobs:
        ...
        eligible_machines = rng.choice(n_machines, n_eligible_machines_per_job, replace=False)
...
def generateSequenceDependantSetupTimes(n_machines, n_jobs, lower_bound, upper_bound, rng):
    ...
    for i in range(n_machines):
        ...
        for j in range(n_jobs):
            plane_coordinates[i,j,:] = rng.integers(low=0.0, high=50.0, size=[2,2]) # 2x2 matrix of coordinates based on paper
...
def generateTimeWindows(n_jobs, tw_density, n_weeks, week_minutes, rng):
    day_minutes = week_minutes/5

    # Generate random indexes for windows
    n_tw = int(tw_density * n_jobs) # number of jobs with time windows
    tw_indexes = rng.choice(n_jobs, size=n_tw, replace=False) # indexes of jobs with time windows

    # Generate random time window lengths based on number of days rather than weeks
    tw_lengths = rng.integers(low=0, high=5*n_weeks, size=n_tw) # lengths of time windows (days)

    # Generate start time based on days rather than weeks
    tw_starts = rng.integers(low=0, high=5*n_weeks-tw_lengths, size=n_tw) # start times of time windows (days)
...
def generateWorkerUnavailability(n_workers, n_weeks, worker_unavailability_numbers, week_minutes, rng):
    day_minutes = week_minutes / 5

    # Generate random indexes for unavailability
    unavailable_indexes = rng.choice(n_workers, size=worker_unavailability_numbers, replace=False)  # indices of unavailable workers

    # Generate random time window lengths based on number of days rather than weeks
    unavailable_lengths = rng.integers(low=0, high=2*n_weeks, size=worker_unavailability_numbers) # lengths of time windows (days) / length of unavailability is maximum 5

    # Generate start time based on days rather than weeks
    unavailable_starts = rng.integers(low=0, high=5*n_weeks-unavailable_lengths, size=worker_unavailability_numbers) # start times of time windows (days)
...
def writeExcel(out_dir, seed, index, n_jobs, n_machines, n_weeks, n_staff, job_proc_times, seq_dep_setup_times, release_times, delivery_times, release_periods, delivery_periods, people_times, machine_eligibilities, job_initial_setup_times, tw_density, worker_unavailability_numbers, weekly_personnel_availability, mean_value, upper_bound, lower_bound, machine_eligibility, personnel_assignments, worker_start_off_times, worker_end_off_times, worker_start_off_periods, worker_end_off_periods, total_unavailable_length):
    ...
    xlsx_path = os.path.join(out_dir, f"Scheduling_Instance_{index}_Seed{seed}.xlsx")
    ...
    # ✅ User requested: Seed number on row 14
    worksheet.write(15, 0, 'Seed Number:')
    worksheet.write(15, 1, seed)
    ...
    worksheet.write(0, 12, 'Time_Begin')
    worksheet.write(1, 12, 'Orders')
    worksheet.write(1, 13, 'Machines')
    for i in range(n_machines):
        worksheet.write(2, i+13, i+1)
    for i in range(n_jobs):
        worksheet.write(i+3, 12, i+1)
        for j in range(n_machines):
            worksheet.write(i+3, j+13, job_initial_setup_times[j, i])

    # Release Weeks
    worksheet.write(0, 24, 'ReleaseWeeks')
    worksheet.write(1, 24, 'Orders')
    worksheet.write(1, 25, 'ReleaseWeek')
    for i in range(n_jobs):
        worksheet.write(i+2, 24, i+1)
        worksheet.write(i+2, 25, release_periods[i])

    # Delivery Weeks
    worksheet.write(0, 27, 'DeliveryWeeks')
    worksheet.write(1, 27, 'Orders')
    worksheet.write(1, 28, 'Deadline')
    for i in range(n_jobs):
        worksheet.write(i+2, 27, i+1)
        worksheet.write(i+2, 28, delivery_periods[i])

    # Release Times
    worksheet.write(0, 30, 'ReleaseTimes')
    worksheet.write(1, 30, 'Orders')
    worksheet.write(1, 31, 'Weeks')
    for i in range(n_weeks):
        worksheet.write(2, i+31, i+1)
    for i in range(n_jobs):
        worksheet.write(i+3, 30, i+1)
        for j in range(n_weeks):
            worksheet.write(i+3, j+31, release_times[i, j])

    # Delivery Times
    worksheet.write(0, 38, 'DeliveryTimes')
    worksheet.write(1, 38, 'Orders')
    worksheet.write(1, 39, 'Weeks')
    for i in range(n_weeks):
        worksheet.write(2, i+39, i+1)
    for i in range(n_jobs):
        worksheet.write(i+3, 38, i+1)
        for j in range(n_weeks):
            worksheet.write(i+3, j+39, delivery_times[i, j])

    # # Worker Start Off Periods
    # worksheet.write(0, 42, 'WorkerStartOffPeriods')
    # worksheet.write(1, 42, 'Worker')
    # worksheet.write(1, 43, 'StartOffPeriod')
    # for i in range(n_staff):
    #     worksheet.write(i+2, 42, i+1)
    #     worksheet.write(i+2, 43, worker_start_off_periods[i])

    # # Worker End Off Periods
    # worksheet.write(0, 45, 'WorkerEndOffPeriods')
    # worksheet.write(1, 45, 'Worker')
    # worksheet.write(1, 46, 'EndOffPeriod')
    # for i in range(n_staff):
    #     worksheet.write(i+2, 45, i+1)
    #     worksheet.write(i+2, 46, worker_end_off_periods[i])

    # # Worker Start Off Times
    # worksheet.write(0, 48, 'WorkerStartOffTimes')
    # worksheet.write(1, 48, 'Worker')
    # worksheet.write(1, 49, 'Weeks')
    # for i in range(n_weeks):
    #     worksheet.write(2, i+48, i+1)
    # for i in range(n_staff):
    #     worksheet.write(i+3, 49, i+1)
    #     for j in range(n_weeks):
    #         worksheet.write(i+3, j+48, worker_start_off_times[i, j])

    # # Worker End Off Times
    # worksheet.write(0, 56, 'WorkerEndOffTimes')
    # worksheet.write(1, 56, 'Worker')
    # worksheet.write(1, 57, 'Weeks')
    # for i in range(n_weeks):
    #     worksheet.write(2, i+56, i+1)
    # for i in range(n_staff):
    #     worksheet.write(i+3, 57, i+1)
    #     for j in range(n_weeks):
    #         worksheet.write(i+3, j+56, worker_end_off_times[i, j])
...
def writeTxt(out_dir, seed, index, n_jobs, n_machines, n_weeks, n_staff, job_proc_times, seq_dep_setup_times, release_times, delivery_times, release_periods, delivery_periods, people_times, machine_eligibilities, job_initial_setup_times, tw_density, worker_unavailability_numbers, weekly_personnel_availability, mean_value, upper_bound, lower_bound, machine_eligibility, personnel_assignments, worker_start_off_times, worker_end_off_times, worker_start_off_periods, worker_end_off_periods, total_unavailable_length):
    ...
    txt_path = os.path.join(
        out_dir,
        f"J{n_jobs}_M{n_machines}_P{n_staff}_W{n_weeks}_TW{tw_density}_ME{machine_eligibility}_AP{worker_unavailability_numbers}_Seed{seed}.txt"
    )
                ...
                if personnel_available < num_machines:
                    ...
                    if num_machines / personnel_available <= 3:
                        ...
                        if worker_unavailability_number <= personnel_available:
                            ...
                            instance_data = {
                                'Instance Number': int(instance_number),
                                'Number of Jobs': num_jobs,
                                'Number of Machines': num_machines,
                                'Number of Weeks': num_weeks,
                                'Personnel Available': personnel_available,
                                'Time Window Density': float(tw_percent),
                                'Worker Unavailability Number': worker_unavailability_number,
                                'Weekly Personnel Availability': weekly_personnel_availability,
                                'Machine Eligibility Constraint' : float(machine_eligibility)
                            }
                            valid_combinations.append(instance_data)
            
    return valid_combinations

                        # instance_details = f"J{num_jobs}_M{num_machines}_P{personnel_available}_W{num_weeks}_TW{tw_percent}_ME{machine_eligibility}_AP{worker_unavailability_number}_new.txt"
                        # excel_file_name = f"Scheduling_Instance_new_{counter}.xlsx"
                        # counter += 1
...
# -------------------------
def generate_one_task(args):
    """
    args = (seed, instance_id, params_dict, out_excel_dir, out_txt_dir)
    """
    seed, index, instance, out_excel_dir, out_txt_dir  = args

    # Deterministic RNG per (seed, instance_id), independent of parallel ordering:
    local_seed = (seed * 1_000_000) + index
    ...
    worker_start_off_times, worker_end_off_times, worker_start_off_periods, worker_end_off_periods, total_unavailable_length = generateWorkerUnavailability(n_staff, n_weeks, worker_unavailability_numbers, week_minutes, rng)
    ...
    job_proc_times, job_initial_setup_times, machine_eligibilities = generateJobProcessingTimes(lower_bound, upper_bound, mean_value, n_machines, n_jobs, machine_eligibility, rng)

    # Generate sequence dependant setup times
    seq_dep_setup_times = generateSequenceDependantSetupTimes(n_machines, n_jobs, lower_bound, upper_bound, rng)

    # Generate time windows
    release_times, delivery_times, release_periods, delivery_periods = generateTimeWindows(n_jobs, tw_density, n_weeks, week_minutes, rng)

    # Write to excel
    excel_filename = writeExcel(out_excel_dir, seed, index, n_jobs, n_machines, n_weeks, n_staff, job_proc_times, seq_dep_setup_times, release_times, delivery_times, release_periods, delivery_periods, people_times, machine_eligibilities, job_initial_setup_times, tw_density, worker_unavailability_numbers, weekly_personnel_availability, mean_value, upper_bound, lower_bound, machine_eligibility, personnel_assignments, worker_start_off_times, worker_end_off_times, worker_start_off_periods, worker_end_off_periods, total_unavailable_length)
    txt_filename = writeTxt(out_txt_dir , seed, index, n_jobs, n_machines, n_weeks, n_staff, job_proc_times, seq_dep_setup_times, release_times, delivery_times, release_periods, delivery_periods, people_times, machine_eligibilities, job_initial_setup_times, tw_density, worker_unavailability_numbers, weekly_personnel_availability, mean_value, upper_bound, lower_bound, machine_eligibility, personnel_assignments, worker_start_off_times, worker_end_off_times, worker_start_off_periods, worker_end_off_periods, total_unavailable_length)

    return{
    "BaseInstanceID": index,
    "Seed": seed,
...
if __name__ == "__main__":
    ...
    valid = build_valid_combinations()

    # Give each base combination a stable id (starting at 0)
    base_start_id = 0

    # Build all tasks: replicate every combination for every seed
    tasks = []
    for seed in seeds:
        for i, params in enumerate(valid):
            instance_id = base_start_id + i   # same id across seeds (seed distinguishes replicate)
            tasks.append((seed, instance_id, params, out_excel_dir, out_txt_dir))

    # Parallel generation
    nproc = min(N_WORKERS, cpu_count())
    print(f"Generating {len(tasks)} instances using {nproc} workers...")

    records = []
    with Pool(processes=nproc) as pool:
        for rec in pool.imap_unordered(generate_one_task, tasks, chunksize=10):
            records.append(rec)
            print(f"Done: Instance {rec['BaseInstanceID']} | Seed {rec['Seed']}")

    records_df = pd.DataFrame(records).sort_values(["BaseInstanceID", "Seed"])
    records_df.to_excel("./data/instance_records_parallel.xlsx", index=False)

    print("✅ Instance generation is complete!")

    print("Instance record file created!")
